"""Sub-minute momentum scalper on M1 bars.

Uses ultra-fast EMA crossovers (3/8) with volume confirmation and
RSI extremes to catch micro-momentum bursts on 1-minute data.
Designed for tight SL/TP and rapid turnover.
"""
...
@register_algorithm
class TickScalper(BaseAlgorithm):
    """Sub-minute momentum scalper for commodity futures.

    Entry: fast EMA (3) crosses slow EMA (8) with volume spike and
    RSI confirmation. Tight risk via metadata-driven SL/TP.
    """
    ...
    def __init__(self, params: dict = None):
        ...
        self.rsi_ob = self.params.get("rsi_overbought", 75)
        self.rsi_os = self.params.get("rsi_oversold", 25)
        self.vol_mult = self.params.get("volume_multiplier", 1.5)
    ...
    def generate_signal(self, data: pd.DataFrame, symbol: str) -> Optional[Signal]:
        ...
        # Volume confirmation: current bar volume > vol_mult * average
        vol_confirmed = True
        if volume is not None and len(volume) > 20:
            avg_vol = volume.tail(20).mean()
            if avg_vol > 0:
                vol_confirmed = volume.iloc[-1] >= self.vol_mult * avg_vol

        # Volume check disabled for M1 — tick volume unreliable at minute level
