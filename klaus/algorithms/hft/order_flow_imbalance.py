"""VPIN-based order flow imbalance signal for HFT.

Based on Easley, Lopez de Prado & O'Hara (2012, Review of Financial Studies).
Uses bulk volume classification (BVC) to estimate buy/sell pressure from
tick-volume on M1 bars, then computes VPIN over rolling volume buckets.
...
@register_algorithm
class OrderFlowImbalance(BaseAlgorithm):
    """VPIN order flow imbalance for high-frequency commodity trading.
    ...
    def __init__(self, params: dict = None):
        ...
        self.vpin_threshold = self.params.get("vpin_threshold", 0.7)
