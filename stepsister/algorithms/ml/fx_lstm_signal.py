"""LSTM sequence model for directional prediction on H1 forex (PyTorch).

Same architecture as Klaus's lstm_signal but targeted at major forex
pairs. Predicts next-bar direction from a sequence of normalised
technical features. Retrains periodically on rolling windows.
"""
...
class _FXLSTMNet(nn.Module):
    """Simple LSTM classifier for forex direction prediction."""
...
class FXLSTMSignal(BaseAlgorithm):
    """LSTM-based directional signal for H1 forex using PyTorch.

    Input: sequence of [returns, vol, RSI, MACD, bb_pct, zscore] over N bars.
    Output: probability of up move.
    Supported on major pairs where intraday autocorrelation is highest.
    """
    ...
    def __init__(self, params: dict = None):
        ...
        self._model: Optional[_FXLSTMNet] = None
    ...
    def fit(self, data: pd.DataFrame) -> None:
        ...
        if len(df) < self.seq_length + 100:
            logger.warning(f"FX LSTM: insufficient data ({len(df)} rows)")
            return
        ...
        torch.save(self._model.state_dict(), MODEL_DIR / "fx_lstm_latest.pt")
        logger.info(f"FX LSTM trained on {split} sequences, last_loss={total_loss / max(n_batches, 1):.4f}")
    ...
    def generate_signal(self, data: pd.DataFrame, symbol: str) -> Optional[Signal]:
        ...
        # Signal strength from sigmoid output distance from 0.5
        if prob_up > 0.6:
